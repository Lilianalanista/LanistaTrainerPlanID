...
Ext.define('LanistaTrainer.controller.PlanController', {
    ...
    onShowPlanPanel: function(planname, callback) {
        ...
        if((plan.data.bu_domain === '' && plan.data.creator_id === '3711') || plan.data.bu_domain !== ''){
            setTimeout(function () {
                Ext.Msg.show({
                    title: Ext.ux.LanguageManager.TranslationArray.MSG_PASSWORD_FORGOT_ERROR_2,
                    message: Ext.ux.LanguageManager.TranslationArray.REGISTRATION_FEATURES,
                    width: 500,
                    height: 200,
                    cls: 'lanista-form-register',
                    buttons: Ext.MessageBox.YESNO,
                    fn: function(buttonId) {
                        if( buttonId == 'yes' ) {
                            registryLink = plan.data.bu_domain === '' ? (plan.data.creator_website !== '' ? (plan.data.creator_website.indexOf('http://') > 0 ? plan.data.creator_website : 'http://' + plan.data.creator_website) : '') :
                            (plan.data.bu_email !== '' ? (plan.data.bu_email.indexOf('http://') > 0 ? plan.data.bu_email : 'http://' + plan.data.bu_email) : '');
